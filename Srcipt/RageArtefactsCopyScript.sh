...
#cd $SRC_DIR/rage
cp -rn $SRC_DIR/rage/codec $SRC_DIR
tar -zcvf codec.tar.gz codec
rm -rf codec/

cp -rn $SRC_DIR/rage/eoplugin $SRC_DIR
tar -zcvf eoplugin.tar.gz eoplugin
rm -rf eoplugin/

#cp -rn $SRC_DIR/rage/rage $SRC_DIR
#tar -zcvf rage.tar.gz rage
#rm -rf rage/

chown -R $USR.$GRP $DST_DIR
#cd $SRC_DIR
cp -rn $SRC_DIR/codec.tar.gz $DST_DIR/roles/wildfly/files
cp -rn $SRC_DIR/eoplugin.tar.gz $DST_DIR/roles/wildfly/files
cp -rn $SRC_DIR/rage/rage.tar.gz $DST_DIR/roles/wildfly/files
...
chown -R $USR.$GRP $DST_DIR/roles/rd-import/files

echo "Done copying rage jar artefacts! +++++++++++++"

rm -rf eoplugin.tar.gz
rm -rf deployments.tar.gz
rm -rf codec.tar.gz
rm -rf certificate.tar.gz
rm -rf copy_to_Wildfly_Server/

echo "++++++++++++++++++++++++ END +++++++++++++"
ldconfig
chown -R $USR.$GRP $SRC_DIR
echo "Done!"
